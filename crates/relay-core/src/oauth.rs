...
pub fn lock(dir: &Path) -> Result<File> {
    storage::open_locked_private(
        &dir.join("credential.lock"),
        Some(std::time::Duration::from_secs(30)),
    )
    .map_err(|_| "credential lock timed out")
}
fn mac() -> bool {
...
fn keychain_account() -> Result<String> {
    ...
    Ok(String::from_utf8_lossy(&user.stdout).trim().to_owned())
}
#[cfg(target_os = "macos")]
fn keychain_read() -> Result<Value> {
    let account = keychain_account()?;
    let bytes =
        security_framework::passwords::get_generic_password("Claude Code-credentials", &account)
            .map_err(|_| "cannot read Keychain")?;
    serde_json::from_slice(&bytes).map_err(|_| "invalid live credentials")
}
#[cfg(not(target_os = "macos"))]
fn keychain_read() -> Result<Value> {
    unreachable!("mac() only returns true when target_os = \"macos\"")
}
// Uses the native Keychain API so the token never appears in a child
// process's argv (as it would via `security add-generic-password -w <token>`).
#[cfg(target_os = "macos")]
fn keychain_write(value: &Value) -> Result<()> {
    let account = keychain_account()?;
    let content = serde_json::to_vec(value).map_err(|_| "cannot encode credentials")?;
    security_framework::passwords::set_generic_password("Claude Code-credentials", &account, &content)
        .map_err(|_| "cannot update Keychain")
}
#[cfg(not(target_os = "macos"))]
fn keychain_write(_value: &Value) -> Result<()> {
    unreachable!("mac() only returns true when target_os = \"macos\"")
}
...
pub fn names(creds: &Path) -> Result<Vec<String>> {
    let order = creds
        ...
        .join("order");
    // Serialize the read-merge-write against other concurrent callers (another
    // `relay` invocation, a daemon tick) so one doesn't clobber the other's write.
    let _lock = storage::lock_config(&order)?;
